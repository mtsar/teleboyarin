    ...
    redis.get(msg.from.id).then(raw => {
        ...
        if (text == '/cancel') {
            ...
            switch (!!state ? state.text : undefined) {
                case '/annotate':
                    stateAnnotate(text, msg, state);
                    break;
                case '/annotate/answer':
                    stateAnnotateAnswer(text, msg, state);
                    break;
                case '/process':
                    stateProcess(text, msg, state);
                    break;
                default:
                    stateInitial(text, msg, state);
                    break;
            }
        }
    });
        ...
        default:
            ...
            if (task.answers.indexOf(text) > -1) {
                ...
                request.patch(`${config.apiURL}/processes/${process.id}/workers/${worker.id}/answers`, {form: answers}).then(response => {
                    ...
                    const markup = JSON.stringify({hide_keyboard: true});
                    bot.sendMessage(msg.chat.id, reply, {parse_mode: 'Markdown', reply_markup: markup}).then(() => {
                        redis.set(msg.from.id, JSON.stringify({text: process.id, processes: [process]})).then(() =>{
                            stateAnnotate(process.id);
                        });
                    });
                });
            }
